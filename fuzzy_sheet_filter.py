...
# allows for google login instead of saved token in program file
...
#sets your defaults for UI input fields
...
SHEET_IDENTIFIER_DEFAULT = 'Paste the Google Sheet URL or name here.'
...
def run_filter_threaded():
    """wraps core logic in a separate thread to keep UI responsive."""
    #disable button while processing
    run_button.config(state=tk.DISABLED, text="Processing...")
    update_status("Starting filtering. Please wait for browser login(first run only)...", 'blue')
...
def run_filter(sheet_id, worksheet_title, column_filter, target_term, threshold):
    ...
    try:
        ...
        #users don't have to log in and the token is stored locally
        ...
        #getting all records and returning data as list of dictionaries(records)
        ...
        output_data = filtered_df[[column_filter, 'Ratio']].reset_index(names=['Original Row Index (0-based)'])
...
tk.Label(input_frame, text="1. Spreadsheet URL or Name").grid(row=row_index, column=0, sticky="w",pady=2)
